...
class ExplorerServices {
    ...
    validatePassword(password, explorer) {
        ...
        const hash = crypto.pbkdf2Sync(password, explorer.salt, iteration, 64, 'sha512').toString('base64');
    ...
    create(explorer) {
        ...
        explorer.hash = crypto.pbkdf2Sync(explorer.password, explorer.salt, iteration, 64, 'sha512').toString('base64');
    ...
    generateJWT(explorer, needNewRefresh = true) {
        let refreshToken = '';
        const accessToken = jwt.sign({ username: explorer.username }, process.env.JWT_TOKEN_SECRET, { expiresIn: process.env.JWT_TOKEN_LIFE }); //Generate the token
    ...
    async validateRefreshToken(username, refreshToken) {
        return await Explorers.findOne({ username: username, refreshToken: refreshToken });
        
    }

    logout(username) {
        return Explorers.findOneAndUpdate({ username: username }, { refreshToken: '' }, { new: true });
    }

    logoutRefresh(refreshToken) {
        return Explorers.findOneAndUpdate({ refreshToken: refreshToken }, { refreshToken: '' }, { new: true });
    }
